...
class DMCMultiCam(embodied.Env):
    ...
    def __init__(self, env, repeat=1, render=True, size=(64, 64), camera=-1):
        ...
        if isinstance(env, str):
            ...
            elif domain == "humanoidmulticam":
                from . import humanoidmulticam

                env = getattr(humanoidmulticam, task)()
                # if task == 'runps': task = 'run_pure_state'
    ...
    def obs_space(self):
        ...
        if self._render:
            spaces["image_back"] = embodied.Space(np.uint8, self._size + (3,))
            spaces["image_side"] = embodied.Space(np.uint8, self._size + (3,))
            spaces["image_ego"] = embodied.Space(np.uint8, self._size + (3,))
        return spaces
    ...
    def step(self, action):
        ...
        if self._render:
            obs["image_back"] = self.render(0)
            obs["image_side"] = self.render(1)
            obs["image_ego"] = self.render(2)
        return obs

    def render(self, cam_id):
        return self._dmenv.physics.render(*self._size, camera_id=cam_id)
