...
class VQCodes(base.Wrapper):
    ...
    def obs_space(self):
        ...
        for key in spaces.keys():
            if re.match(self.vqcodes, key):
                ...
                new_spaces[f"embed_{key}"] = spacelib.Space(
                    self.reward_model.embed_dtype, self.reward_model.embed_size
                )
    ...
    def step(self, action):
        ...
        encodings, embeddings = self.reward_model.ae.encode(
            images_to_encode, include_embeddings=True
        )
        new_obs = {}
        ...
        for key, val in obs.items():
            if re.match(self.lang_key, key):
                if val not in self.cache:
                    ...
                    text_tokens, text_mask = self.tokenize_with_mask(
                        val, context_length=16
                    )
